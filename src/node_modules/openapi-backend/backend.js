...
class OpenAPIBackend {
    ...
    constructor(opts) {
        ...
        this.securityHandlers = Object.assign({}, optsWithDefaults.securityHandlers); // Copy to avoid mutating passed object
    ...
    async init() {
        try {
            ...
            if (typeof this.inputDocument === 'string') {
                this.definition = (await json_schema_ref_parser_1.dereference(this.inputDocument));
            }
            else {
                this.definition = (await json_schema_ref_parser_1.dereference(this.document || this.inputDocument));
            }
        }
        catch (err) {
            if (this.strict) {
     ...
     */
    async loadDocument() {
        this.document = (await json_schema_ref_parser_1.parse(this.inputDocument));
    ...
    async handleRequest(req, ...handlerArgs) {
        ...
        const response = await (async () => {
            ...
            context.security = Object.assign({ authorized }, securityHandlerResults);
